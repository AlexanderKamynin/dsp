...
class DigitalSignalProcessing:
    ...
    def start(self) -> NoReturn:
        args = CLI.choose_generator()
        self.__signal_generator = SignalGenerator(*args['param'])
        if args['generator_type'] == 1:
            self.__signal_sequence = self.__signal_generator.generate_signal(*args['time'])
        elif args['generator_type'] == 2:
            self.__signal_sequence = self.__signal_generator.generate_noise_signal(*args['time'])
        ...
        window_size = CLI.input_window_size()
        self.__filter = Filter(window_size)
        ...
        args = CLI.choose_autocorrelation_func()
        if args == 1:
